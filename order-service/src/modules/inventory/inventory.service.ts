...
export class InventoryService {
  ...
  async getShardIndexByInventoryId(inventoryId: number) {
    const shardIndexCacheKey = `inventory:${inventoryId}:shard_index`;
    const keyRedisClient = this.redisService.getClient(0);
    const shardIndexCache = await keyRedisClient.get(shardIndexCacheKey);
    if (shardIndexCache) {
      return parseInt(shardIndexCache, 10);
    } else {
      ...
      const shardIndex = inventory.shard_index;
      await keyRedisClient.set(shardIndexCacheKey, shardIndex, 'EX', 3600);
  ...
  async purchaseInventories(inventories: any[]) {
    ...
    if (inventories.length === 0 || inventories[0].inventory_id === undefined) {
      throw new Error('Failed to purchase inventories');
    }
    const shardIndex = await this.getShardIndexByInventoryId(
    ...
    const result = await this.redisService.evalClient(
      shardIndex,
      luaScript,
      keys,
      args,
  ...
  async createInventory(inventory: any, shardIndex: number) {
    ...
    await redisShardClient.hset(cacheKey, 'price', inventory.price.toString());
    // await redisShardClient.hset(
    //   cacheKey,
    //   'discount',
  ...
  async createManyInventories(inventories: any[], shardIndex: number) {
    let newInventories = [];
    ...
    // Lưu thông tin của từng inventory vào Redis sharding
    const redisShardClient = this.redisService.getClient(shardIndex);
  ...
  private async getInventoryFromCache(inventory: any) {
    const cacheKey = `inventory:${inventory.inventory_id}`;
    const shardIndexCache = await this.getShardIndexByInventoryId(
      inventory.inventory_id,
    );
    const redisShardClient = this.redisService.getClient(shardIndexCache);
    //console.log('Getting inventory from cache:', cacheKey);
    const cacheData: any = await redisShardClient.hgetall(cacheKey);
  ...
  async updateFlashSalePriceAndQuantityAndTime(
    ...
    const shardIndex = await this.getShardIndexByInventoryId(inventoryId);
    const redisShardClient = this.redisService.getClient(shardIndex);
    await redisShardClient.hset(
      cacheKey,
      'flash_sale_price',
  ...
  async deleteInventoriesByProductId(productId: string) {
    ...
    for (let i = 0; i < inventories.length; i++) {
      ...
      await redisShardClient.del(cacheKey);
    }
  ...
  async updatePriceAndQuantity(
    ...
    const shardIndex = await this.getShardIndexByInventoryId(inventoryId);
    const redisShardClient = this.redisService.getClient(shardIndex);
    await redisShardClient.hset(cacheKey, 'price', price.toString());
    await redisShardClient.hset(cacheKey, 'quantity', quantity.toString());
  }
  ...
  async updateInventoryByProductId(
    ...
    const deleteOldAndCreateNewInventories = async (productId, inventories) => {
      ...
      let newInventories = await this.createManyInventories(
        inventories,
        shardIndex,
      );
    ...
    if (new_classifications.length === 0 && old_classifications.length !== 0) {
      await this.deleteInventoriesByProductId(productId);
      let newInventory = await this.createInventory(inventories, shardIndex);
      return newInventory;
    }
    ...
    // Case 3: if new classification's length == old classification's length
      ...
      if (new_classifications.length === 1) {
        ...
        // Case 3.1.2: if new classification's id == old classification's id
          ...
          await this.createManyInventories(toCreate, shardIndex);
          return true;
        }
      }
