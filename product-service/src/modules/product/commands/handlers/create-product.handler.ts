...
export class CreateProductHandler implements ICommandHandler<CreateProductCommand> {
  ...
  async execute(command: CreateProductCommand) {
    ...
    //upload files
    const productImages = product.files ? await this.fileService.uploadFiles(product.files) : [];

    const category = await this.categoryRepository.findOneById(product.category_id)
    if (!category) {
    ...
    for (const classification of product.classifications) {
      ...
      newProduct.classifications.push(new Types.ObjectId(newClassification._id));
    }

    await this.productRepository.update(newProduct._id, newProduct)
    let returnProduct = await this.productRepository.findOneByIdWithPopulate(newProduct._id, ['category', 'classifications']);
    ...
    inventories = inventories.map(inventory => {
      ...
      inventory.classification_main_id = this.searchItemNameToId(classifications, inventory.classification_main_id);
      inventory.classification_sub_id = this.searchItemNameToId(classifications, inventory.classification_sub_id);
      return inventory;
    });
    let createdInventories = await this.inventoryService.createManyInventories(inventories);
    // @ts-ignore
